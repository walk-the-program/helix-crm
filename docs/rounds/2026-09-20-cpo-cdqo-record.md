...
| F-P2-LA-2 | critical | defect | After the `fit` close, Contacts renders 10 of 16 contacts at 1280x820 (12 of 16 compact) with no scrollbar while the header says "16 of 16 people": `fit` sizes the scroller from what the virtualiser has measured and nothing above constrains it, so it settles at one viewport and never grows; the panel is overflow-hidden. The 3-contact seed in listNav.e2e.ts passes under every broken arrangement. Reported by lead-records; **not reproduced by the coordinator on committed HEAD** (`cpoRows.e2e.ts`, port 4222, sample week at 1280x820: 16 of 16 rendered in both densities, the list is overflow auto and scrolls to the last contact, `rows-comfortable.png`). Resolution: both measurements were right about different arrangements. Committed HEAD still carried the half-adopted panel (`flex-1` kept), in which `fit` did nothing and the slab remained; the arrangement that removes the slab (no `flex-1`) existed only in lead-records' tree and there `fit` v1 measured its available height from the very panel it sizes (the ancestor walk accepted `overflow: hidden`), so the list settled at one screen. Fixed in `1d3763f`: measure only from a genuinely scrolling ancestor (`<main>`), clamp to the viewport, re-measure on the next frame; the two panels drop `flex-1` in the same commit. 40-contact guard green in both densities (listNav + records 28 passed); coordinator's `cpoRows.e2e.ts` re-run green. | closed | lead-platform, lead-records | implemented |

| F-LB-D1..D26 | — | design | Lead B's phase-two findings (`docs/rounds/cpo-returns/lead-b-money-design.md`): em dash for period zeros at the shared DataTable; two-line caption; one headline system; Copy as CSV on the per-deal table; 1024 overflow cue; quiet empties; Invoices field order, sub-rows, tax column only when mixed, number column at 1024; Document page actions hierarchy (Send is a draft action; a draft offers Delete only); repositories say the owner's word. | all implemented; D8 closed in `460a201` (aging buckets as a kit Table with the bucket as a word, empty buckets kept with an em dash because the shape of the ageing is the report, the total inside the table as the one primary block) | lead-money | implemented |
...
| F-LA-D1..D12 + W1/W2 | — | design | Lead A's phase-two findings (`docs/rounds/cpo-returns/lead-a-p2-design.md`): "No company" → em dash; board subtotals; deals list shows the customer; vocabulary in the stage manager and empties; empty lists hide the toolbar; board empty state; expected/won date back in the deal header (self-correction); contact rail leads with jobs; row links not underlined as fragments; "Move to <Stage>"; validation reads vocabulary; slab closed by `fit`. Workers: two CRITICAL 1024-px reachability bugs (Trash's 13 tabs ran off-screen with no scroll `62a5067`; Reminders action column clipped `0761248`); disabled Delete-forever explanation; RuleDialog validation timing; Templates merge-field glossary; tag-before-name on Today rows; standing sentences → tooltips; quick add tablist keyboard nav; search dialog on the kit overlay so ⌘K and ⌘⇧K match (`2b5c627`). | implemented | lead-records | implemented |
| F-P2-LA-3 | low | kit | Raised by Lead A, not patched: DatePicker accepts `aria-invalid` without an invalid border style; `Kbd` has no accessible label. | routed to lead-platform (kit) | lead-platform | in progress |
| F-P2-LA-4 | low | deferred | Search `GROUP_HEADINGS.deal` and the zod `newDealSchema` messages are fixed at module load and ignore the vocabulary. | deferred: a module-load constant cannot read a workspace setting; the heading and the messages read "Deal" only in a non-"deals" workspace's search dialog and zod paths, which the vocabulary-aware ValidationError messages already cover for the common cases | lead-records | deferred |
| F-P2-LA-5 | low | decision | Template merge tokens `{{deal_title}}` / `{{deal_value}}` in a "jobs" workspace. | ruling: token names are stable identifiers and are never renamed; the glossary describes each token in the workspace's word | lead-records | closed by ruling |
